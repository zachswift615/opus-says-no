...
description: Create an implementation plan from a design document using the blueprint skill
arguments:
...
Create a comprehensive implementation plan from the design document using the **blueprint** skill.
...
3. **Choose planning skill:**
   - **8+ tasks or complex?** → Use `blueprint-maestro` (recommended)
   - **< 5 simple tasks?** → Use `blueprint`
...
**With blueprint-maestro (recommended):**
...
**With blueprint (simple plans only):**
1. Create task outline
2. Gap analysis with Opus subagent
...
**Default to maestro when in doubt** - it handles any plan size gracefully.
